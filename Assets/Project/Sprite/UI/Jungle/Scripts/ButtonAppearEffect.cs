using UnityEngine;
using System.Collections;
using System.Collections.Generic;

public class ButtonAppearEffect : MonoBehaviour {

	// use them causing strange NaN problem. Probably unity's bug. So use their actual value's in the code.

	public float duration=0.5f;

	public float rotationPercent=50;
	public float shrinkPercent=30;


	public Quaternion originalRotation;
	private Vector3 originalScale;
	private float shrinkPercentFloat;
	private float rotationPercentFloat;

	private List<UI2DSprite> fadingSprites= new List<UI2DSprite> ();
	void Start () {
		originalScale = transform.localScale;
		shrinkPercentFloat = shrinkPercent / 100f;
		rotationPercentFloat = rotationPercent / 100f;
		addFadingSprites (transform);
	}

	void addFadingSprites(Transform root){
		try {
			fadingSprites.Add (root.GetComponent<UI2DSprite> ());
		} catch (System.Exception e){
		}
	}

	void ChangeAllAphaTo(float alpha){
		try {
			/*
			foreach (UI2DSprite sprite in fadingSprites) {
				sprite.color = new Color (1, 1, 1, alpha);
			}
			*/
		}
		catch (System.Exception e){
		}
	}

	private float rotationLeft;
	private float rotationSpeed;
	// Update is called once per frame
	void Update () {
		if (effectIsHappenning) {
			if (Time.fixedTime >= startTime + duration) {
				effectIsHappenning = false;
				transform.rotation = originalRotation;
				transform.localScale = originalScale;
				ChangeAllAphaTo (1);
			} else {
				float currentStage = (Time.fixedTime - startTime) / 0.5f;//duration;
				if (currentStage < rotationPercentFloat) {
					// Rotate
					float currentRotation = Time.deltaTime * rotationSpeed;
					if (rotationLeft > currentRotation)
					{
						rotationLeft-=currentRotation;
					}
					transform.Rotate(0f,0f,currentRotation);

					ChangeAllAphaTo (Mathf.SmoothStep (0, 1, currentStage * 2));
				} else {
					ChangeAllAphaTo (1);
					// Scale
					transform.rotation = originalRotation;
					float currentScaleStage = (Time.fixedTime - startTime - rotationPercentFloat * duration) / ((1-rotationPercentFloat) * duration);
					if (currentScaleStage < 0.5) {
						// Shrink
						/*
						transform.localScale = new Vector3 (
							Util.EaseOut(currentScaleStage, originalScale.x, -shrinkPercentFloat*originalScale.x, (1-rotationPercentFloat)*duration/2),
							Util.EaseOut(currentScaleStage, originalScale.y, -shrinkPercentFloat*originalScale.y, (1-rotationPercentFloat)*duration/2),
							1);
							*/
						transform.localScale = new Vector3 (
							Mathf.Lerp (originalScale.x, (1f - shrinkPercentFloat) * originalScale.x, currentScaleStage * 2f),
							Mathf.Lerp (originalScale.y, (1f - shrinkPercentFloat) * originalScale.y, currentScaleStage * 2f),
							1);

					} else {
						// Expand
						/*
						transform.localScale = new Vector3 (
							Util.EaseOut(currentScaleStage, (1-shrinkPercentFloat)*originalScale.x, shrinkPercentFloat*originalScale.x, (1-rotationPercentFloat)*duration/2),
							Util.EaseOut(currentScaleStage, (1-shrinkPercentFloat)*originalScale.y, shrinkPercentFloat*originalScale.y, (1-rotationPercentFloat)*duration/2),
							1);
						*/
						transform.localScale = new Vector3 (
							Mathf.Lerp ((1f - shrinkPercentFloat) * originalScale.x, originalScale.x, (currentScaleStage-0.5f) * 2f),
							Mathf.Lerp ((1f - shrinkPercentFloat) * originalScale.y, originalScale.y, (currentScaleStage-0.5f) * 2f),
							1);
					}
				}
			}
		}
	}

	private bool effectIsHappenning=false;
	private float startTime = 0;
	public void Trigger() {
		startTime = Time.fixedTime;
		effectIsHappenning = true;
		rotationLeft = 90f;
		transform.Rotate (0, 0, -90);
		
		rotationSpeed = 90/(0.5f*0.5f);//(duration*rotationPercentFloat);
		ChangeAllAphaTo (0);
	}


}
